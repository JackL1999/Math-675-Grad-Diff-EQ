\documentclass[12pt]{article}                         
\pagestyle{plain} %-- Document Set Up Command

%-- Document Setup & Layout
\usepackage{fancyhdr}
\usepackage{comment}
\usepackage{multicol}
\usepackage[
  letterpaper,
  left=0.8in,
  right=0.8in,
  textheight=9.5in,
  bmargin=0.75in  % Adjust this value to push the footer down
]{geometry}

%-- Color & Text Formatting
\usepackage[svgnames]{xcolor} 
\usepackage{textcomp}
\usepackage{ulem}
\usepackage{graphicx}

%-- Math Packages
\usepackage{amsmath}     
\usepackage{amsfonts}
\usepackage{amstext}
\usepackage{amssymb}
\usepackage{latexsym}
\usepackage{bm}

%-- Graphics and Figures
\usepackage{graphicx}    
\usepackage{tikz}
\usepackage{pgfplots}
\usepackage{circledtext}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usetikzlibrary{decorations.markings, arrows.meta}
%-- Tables
\usepackage{array}      
\usepackage{tabularx}

%-- Lists
\usepackage{enumitem}    
%\usepackage{enumerate}
\usepackage{tasks}

%-- TikZ Libraries
\usetikzlibrary{shapes.geometric} 
\pgfplotsset{compat=1.18}
\tikzset{
    phase arrow/.style={
        postaction={decorate,
            decoration={
                markings,
                mark=at position 0.1 with {\arrow{Stealth}},
                mark=at position 0.3 with {\arrow{Stealth}},
                mark=at position 0.7 with {\arrow{Stealth}},
                mark=at position 0.9 with {\arrow{Stealth}}
            }
        }
    },
    phase arrow left/.style={
        postaction={decorate,
            decoration={
                markings,
                mark=at position 0.1 with {\arrow{Stealth[reversed]}},
                mark=at position 0.3 with {\arrow{Stealth[reversed]}},
                mark=at position 0.7 with {\arrow{Stealth[reversed]}},
                mark=at position 0.9 with {\arrow{Stealth[reversed]}}
            }
        }
    }
}
%-- Header/Footer
\pagestyle{fancy}
\fancyhf{} % Clear all header and footer fields
\fancyhead[L]{Jack Lingbeck} % Left header with name
\fancyhead[R]{May 13th, 2026} % Right header with date
\renewcommand{\headrulewidth}{0.4pt} % Horizontal line below the header
\rfoot{Page \thepage}
\lfoot{Math-675, Homework 5}

\begin{document}
\large
\begin{center}
\fbox{\begin{tabular}{c}
\bf Math 675 Spring 2026: \\
\bf Homework 4 (LATE)\\
\bf Due Tuesday, April 6th 2026\\
\end{tabular}}
\end{center}

\section*{Question 1:}
Consider the matrix $A = \begin{pmatrix} -5 & 1 \\ 4 & -2 \end{pmatrix}$ \\
The eigenvalues for A are
$\lambda_1 = -1$ and
$\lambda_2 = -6$ with corresponding eigenvectors 
$\mathbf{v}_{\lambda_1} = (1, 4)$ and 
$\mathbf{v}_{\lambda_2} = (-1, 1)$.
Use this and the method of undetermined coefficients to find the general solution to the inhomogeneous system:
\[\mathbf{x}'(t) = \begin{pmatrix} -5 & 1 \\ 4 & -2 \end{pmatrix} x(t) + e^{2t}\begin{pmatrix} 6 \\ -1 \end{pmatrix}\]

\noindent
Our general solution so far is: $\mathbf{x}'(t) = c_1 e^{-1t} \begin{pmatrix} 1 \\ 4 \end{pmatrix} + c_2 e^{-6t} \begin{pmatrix} -1 \\ 1 \end{pmatrix}$ \\
but with this extra inhomogeneous part, lets make an anstatz of: $x_p(t) = we^{2t}$ since the inhomogeneous part has a $e^{2t}$, now plug our guess into the system:
\[2\begin{pmatrix} w_1 \\ w_2 \end{pmatrix} e^{2t} = \begin{pmatrix} -5 & 1 \\ 4 & -2 \end{pmatrix} \begin{pmatrix} w_1 \\ w_2 \end{pmatrix} e^{2t} + \begin{pmatrix} 6 \\ -1 \end{pmatrix}e^{2t}\]
Rearrange and expand a bunch, dropping the $e^{2t}$ terms from both sides, gives:
\[\begin{pmatrix} 2w_1 \\ 2w_2 \end{pmatrix} = \begin{pmatrix} -5w_1 + w_2 \\ 4w_1 - 2w_2 \end{pmatrix} + \begin{pmatrix} 6 \\ -1 \end{pmatrix} \]
\[\begin{pmatrix} 7 & -1 \\ -4 & 4 \end{pmatrix} \begin{pmatrix} w_1 \\ w_2 \end{pmatrix} = \begin{pmatrix} 6 \\ -1 \end{pmatrix}
\xrightarrow{augment}
\left(\begin{array}{cc|c}
    7 & -1 & 6 \\
    -4 & 4 & -1
\end{array}\right) \]
\[\left(\begin{array}{cc|c}
    7 & -1 & 6 \\
    -4 & 4 & -1
\end{array}\right)
\xrightarrow{RREF}
\left(\begin{array}{cc|c}
    1 & 0 & 23/24 \\
    0 & 1 & 17/24
\end{array}\right)
\]
So we obtained that $w_1 = 23/24$ and $w_2 = 17/24$
So our final solution now is:
\[\mathbf{x}'(t) = c_1 e^{-1t} \begin{pmatrix} 1 \\ 4 \end{pmatrix} + c_2 e^{-6t} \begin{pmatrix} -1 \\ 1 \end{pmatrix} + \frac{1}{24}\begin{pmatrix} 23 \\ 17 \end{pmatrix} e^{2t}\]

\vspace{5cm}

\section*{Question 2:}
Consider the matrix $A = \begin{pmatrix} 2 & -1 \\ 3 & 2 \end{pmatrix}$ \\
The eigenvalues for A are
$\lambda_1 = -1$ and
$\lambda_2 = 1$ with corresponding eigenvectors 
$\mathbf{v}_{\lambda_1} = (1, 3)$ and 
$\mathbf{v}_{\lambda_2} = (1, 1)$.

\begin{enumerate}  
    \item \textbf{Calculate} $\boldsymbol{\exp(tA)}$ \\
    Given the definition: $\exp(tA) = \Psi(t) \Psi^{-1}(0)$, where $\Psi(t)$ is the fundamental matrix made up of the $x$ and $y$ solution vectors from $\mathbf{x}(t)$. \\
    First, build the general solution: 
    \[ \mathbf{x}(t) = c_1 e^{-t} \begin{pmatrix} 1 \\ 3 \end{pmatrix} + c_2 e^{t} \begin{pmatrix} 1 \\ 1 \end{pmatrix} \]
    \[\Psi(t) = \begin{pmatrix} e^{-t} & e^{t} \\ 3e^{-t} & e^{t}\end{pmatrix}\]
    \[\Psi^{-1}(0) = (\Psi(0))^{-1} = \begin{pmatrix} 1 & 1 \\ 3 & 1 \end{pmatrix}^{-1} = \frac{1}{2} \begin{pmatrix} -1 & 1 \\ 3 & -1 \end{pmatrix}\]
    \[ \exp(tA) = \Psi(t)\Psi^{-1}(0) = \begin{pmatrix} e^{-t} & e^{t} \\ 3e^{-t} & e^{t}\end{pmatrix} \frac{1}{2} \begin{pmatrix} -1 & 1 \\ 3 & -1 \end{pmatrix}\]
    \[ \exp(tA) = \frac{1}{2} \begin{pmatrix} -e^{-t} + 3e^{t} & e^{-t} - e^{t} \\ -3e^{-t}+3e^{t} & 3e^{-t} - e^{t}\end{pmatrix}\]
    Now, optionally, we could convert this into a form involving hyperbolic trig:
    \[ \exp(tA) = \begin{pmatrix} \sinh(t) & -\sinh(t) \\ 3\sinh(t) & \cosh(t) - 2\sinh(t) \end{pmatrix}\]
    \item \textbf{Solve the IVP} \\
    Use the result from part (1) to solve:
    \[ \mathbf{x}'(t) = \begin{pmatrix} 2 & -1 \\ 3 & -2 \end{pmatrix} \mathbf{x}(t), \quad \mathbf{x}(0) = \begin{pmatrix} 1 \\ 2 \end{pmatrix} \]
    Defintion: Given a system $\mathbf{x}'(t) = Ax(t)$ the solution is $\mathbf{x}(t)=e^{tA}x(0)$
    \[\mathbf{x}(t) = e^{tA}x(0) = \frac{1}{2} \begin{pmatrix} -e^{-t} + 3e^{t} & e^{-t} - e^{t} \\ -3e^{-t}+3e^{t} & 3e^{-t} - e^{t}\end{pmatrix} \begin{pmatrix} 1 \\ 2 \end{pmatrix}\]
    \[\mathbf{x}(t) = e^{tA}x(0) = \frac{1}{2} \begin{pmatrix} -e^{-t} + 3e^{t} + 2e^{-t} - 2e^{t} \\ -3e^{-t} + 3e^{t} + 6e^{-t} - 2e^{t} \end{pmatrix}\]
    \[\mathbf{x}(t) = e^{tA}x(0) = \frac{1}{2} \begin{pmatrix} e^{-t} + e^{t} \\ 3e^{-t} + e^{t} \end{pmatrix}\]
    \[\mathbf{x}(t) = \frac{1}{2}e^{-t}\begin{pmatrix} 1 \\ 3 \end{pmatrix} + \frac{1}{2} e^{t} \begin{pmatrix} 1 \\ 1 \end{pmatrix}\]
    Ta-Da! This is the same IVP Solution as if we solved this the regular way!
\end{enumerate}

\section*{Question 3:}
The following questions refer to the proof of existence and uniqueness covered in chapter 2.3 of the guided notes.
\begin{enumerate}
    \item[a)] \textbf{Why do we choose a small time interval?} \\
    In the proof we chose $\alpha < min\left\{\frac{\rho}{M},\frac{1}{B}\right\}$
    \begin{enumerate}
        \item Explain in words why we need the condition $\alpha < \frac{\rho}{M}$ \\
        We need this in order to bound the first iteration by $\rho$ similarly as $\varepsilon$
        \[\Vert x_1(t) - x_0 \Vert = \Vert t \Vert \cdot \Vert \mathbf{F}(x_0) \Vert \leq \alpha M \leq \frac{\rho}{M} \cdot M \leq \rho\]
        This makes sure that $\forall k, x_k$ remains inside $\mathcal{O}_{\rho}$ centered about $x_0$
        
        \item Explain in words why we need the condition $\alpha < \frac{1}{B}$\\
        We need this in order for the normed difference of sequential iterations to be less than epsilon for convergence. 
        So the $(\alpha B)^k$ term below can get small as we let $k$ be big, which means the iterations get closer together
        \[\Vert \mathbf{x}_{k+1}(t) - \mathbf{x}_k(t) \Vert \leq (\alpha B)^k \cdot L\]
        Later on, we use $\gamma = \alpha B < 1$ so that we can build a convergent geometric series to compare with the normed telecoping series of iterations
        
        \item What could go wrong if $\alpha$ was too large? \\
        This could cause $x_k$ to leave $\mathcal{O}_{\rho}$, which may not guarentee convergence for the ODE solution, and the geometric series $\sum \gamma$ may not converge
    \end{enumerate}

    \item[b)] \textbf{Staying inside the closed ball} \\
    Suppose we have $x_{k+1}(t) = x_0 + \int_{0}^{t} F(x_k(x))ds$
    \begin{enumerate}
        \item Why is it important that all iterates stay inside $\mathcal{O}_{\rho}$ \\
        If we stay $\mathcal{O}_{\rho}$, we then have keep the Lipschitz condition true, \\ we can bound the slope of $\mathbf{F}$ on the domain

        \item What assumptions about $F$ would fail if an iterate left $\mathcal{O}_{\rho}$?\\
        If $x' \notin \mathcal{O}_{\rho}$ then $\Vert \mathbf{F}(x') \Vert$ would not be bounded by $M$

        \item Explain how the bound $\Vert F(w) \Vert \leq M$ is used in this step \\
        Like before, we can use this to bound the first normed difference and the following terms recussively
        \[\Vert x_1(t) - x_0 \Vert = \Vert t \Vert \cdot \Vert \mathbf{F}(x_0) \Vert \leq \alpha M \leq \frac{\rho}{M} \cdot M \leq \rho\]

    \end{enumerate}

    \item[c)] \textbf{Role of Lipschitz Condition} \\
    The proof uses $\Vert F(x) - F(y) \Vert \leq B \Vert x - y \Vert$
    \begin{enumerate}
        \item Where in the proof is this condition used? \\
        \[\Vert \mathbf{x}_2(t) - \mathbf{x}_1(t) \Vert \leq \int_{0}^{t} \Vert \mathbf{F}(\mathbf{x}_1(s)) - \mathbf{F}(\mathbf{x}_0(s)) \Vert \, ds \]
        \[ \leq \int_{0}^{t} B \Vert \mathbf{x}_1(s) - \mathbf{x}_0 \Vert \, ds \leq B \cdot t \cdot L \leq \alpha \cdot B \cdot L\]

        \item Why is continuity of $\mathbf{F}$ alone not enough for the argument? \\
        Would not guarentee that the slope of $\mathbf{F}$ won't be too large or blow up.
        The best we would be able to do is bound $\Vert \mathbf{F}(x_{k+1}) - \mathbf{F}(x_k) \Vert$ by 2$M$ by triangle inequality,
        yet we also have no guarentee that $\sum (\alpha M)^k$ would not blows up to infinity.

        \item In your own words, what does the Lipschitz condition guarentee?\\
        This condition allows us to always bound $\Vert x_{k+1} - x_{k} \Vert$ by the $(\alpha B)^k L$ term.
        Which, as $k \rightarrow \infty$, then $\Vert x_{k+1} - x_{k} \Vert \rightarrow 0$

    \end{enumerate}

    \item[d)] \textbf{Why does a geometric series appear?} \\
    We obtained an estimate $\Vert x_{k+1}(t) - x_k(t) \Vert \leq (\alpha B)^k L$
    \begin{enumerate}
        \item Why is it important that $\alpha B < 1$? \\
        So that the geometric series $\sum (\alpha B)^k L \leq \sum (\gamma)^k L$ converges

        \item What would happen if $\alpha B \geq 1$ \\
        This would result in $\sum (\alpha B)^k$ not converging and thus $\sum \Vert x_{k+1} - x_k \Vert \nless \varepsilon$, meaning the need lemma for convergence could not be applied
        \vspace{4cm}
        \item How does this estimate lead to convergence of $\{x_k\}$ 
        \[\sum_{k=N}^{\infty} \Vert x_{k+1} - x_k \Vert = \Vert x_{N+1} - x_N \Vert + \Vert x_{N+2} - x_{N+1} \Vert + \dots \] 
        \[\hspace*{5cm} \le (\alpha B)^N L + (\alpha B)^{N+1} L + \dots = \sum_{k=N}^{\infty} (\alpha B)^k L < \varepsilon\] 
        since $(\alpha B) < 1$, RHS converges by Cauchy Conditions, so LHS does too.
        and since LHS sum converges to less than epsilon, then there exists a function that $\{x_k\}$ converges to. So we now have $\{x_k(t)\} \rightarrow x(t)$
    \end{enumerate}

    \item[e)] \textbf{Conceptual comparison} \\
    Compare:
    \begin{itemize}
        \item Solving an algebric equation $x = G(x)$ via fixed point iteration
        \item Solving the integral equation $x(t) = x_0 + \int_{0}^{t} F(x(s))ds$
    \end{itemize}
    \begin{enumerate}
        \item What plays the role of the function being iterated? \\
        With Fixed point iteration, we want to obtain a single value for $x^*$ that 
        gets iterated from $x_k$. But in Picard iteration, we want to obtain some 
        sized vector $\vec{x}^*$ from iterating that same sized vector $\vec{x_k}$

        \item What plays the role of the fixed point? \\
        Like above, we want $x^*$ or $\vec{x}^*$ as the fixed point as $k \rightarrow \infty$

        \item Why is this analogy useful? \\
        This allows us to connect multiple fields in math that we have learned. 
        Like how a fixed point problem could be turned into a root finding problem. 
        Now an system of ODE became a fixed point problem.
        Yet also gives us a better conceptual of whats actually going on behind the scenes.
        

    \end{enumerate}
\end{enumerate}

\end{document}

